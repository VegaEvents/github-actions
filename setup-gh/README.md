...
`secrets.GITHUB_TOKEN` is automatically generated by GitHub for every workflow run — no one needs to manually create this secret. However, composite actions can't access the caller's secrets implicitly, so the token must be passed explicitly via the `gh-token` input.
